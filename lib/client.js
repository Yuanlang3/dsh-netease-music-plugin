window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Embedded QR-code generator (qrcode-generator UMD). It assigns
    // module.exports, so save/restore the plugin exports around it.
    var __qrSaved = module.exports;
    /**
...
//# sourceMappingURL=/sm/26b4b0d0b1e283d6b3ec9857ac597d7a60c76ac17be1ef4c965f03086de426bb.map
    var qrcodeFn = module.exports;
    module.exports = __qrSaved;
    ...
    var mini = false;
    var miniSubs = [];
    function setMini(v) { mini = v; for (var i = 0; i < miniSubs.length; i++) miniSubs[i](); }
    function useMini() {
      var st = React.useState(mini);
      var v = st[0];
      var setV = st[1];
      React.useEffect(function () {
        var f = function () { setV(mini); };
        miniSubs.push(f);
        return function () { var idx = miniSubs.indexOf(f); if (idx >= 0) miniSubs.splice(idx, 1); };
      }, []);
      return v;
    }
    ...
    function NmFooterButton(props) {
      ...
      return e("button", {
        type: "button",
        style: { width: 36, height: 36, borderRadius: "50%", background: (isOpen || isMini) ? "rgba(212,60,51,.25)" : "transparent", color: "#e0483e", border: "none", cursor: "pointer", fontSize: 16, display: "inline-flex", alignItems: "center", justifyContent: "center", flex: "none", lineHeight: 1, padding: 0 },
        title: isMini ? "展开网易云音乐" : "网易云音乐",
        onClick: function () {
          if (isOpen || isMini) { setOpen(false); setMini(false); }
          else { setOpen(true); }
        }
      }, "♪");
    }
    ...
    function PlayerRoot() {
      ...
      var qrUrlSt = React.useState(""); var qrUrlVal = qrUrlSt[0]; var setQrUrl = qrUrlSt[1];
      var qrStatusSt = React.useState(""); var qrStatusVal = qrStatusSt[0]; var setQrStatus = qrStatusSt[1];
      var qrLoadingSt = React.useState(false); var qrLoadingVal = qrLoadingSt[0]; var setQrLoading = qrLoadingSt[1];

      var isMini = useMini();
      var posSt = React.useState({ x: 64, y: 60 }); var pos = posSt[0]; var setPos = posSt[1];
      var miniPosSt = React.useState(function () {
        return { x: Math.max(0, (window.innerWidth || 1200) - 292), y: Math.max(0, (window.innerHeight || 800) - 76) };
      }); var miniPos = miniPosSt[0]; var setMiniPos = miniPosSt[1];

      // Drag handler: mousedown on a header, then track mousemove on document
      // and clamp inside the viewport. minW/minH keep part of the window
      // reachable so it can always be grabbed again.
      function startDrag(ev, from, setTo, minW, minH) {
        if (ev.button !== 0) return;
        ev.preventDefault();
        var sx = ev.clientX, sy = ev.clientY, ox = from.x, oy = from.y;
        function onMove(e2) {
          var nx = Math.max(0, Math.min((window.innerWidth || 1200) - minW, ox + e2.clientX - sx));
          var ny = Math.max(0, Math.min((window.innerHeight || 800) - minH, oy + e2.clientY - sy));
          setTo({ x: nx, y: ny });
        }
        function onUp() {
      ...
      React.useEffect(function () {
        if (!qrUrlVal || loggedIn) return;
        var timer = setInterval(function () {
          var key = qrUrlVal.split("codekey=")[1] || "";
          if (!key) return;
          api("GET", "/api/netease/qr-check?key=" + encodeURIComponent(key)).then(function (res) {
            if (res && res.ok) {
              if (res.code === 803 || res.loggedIn) {
                ...
                setQrStatus((res.message || "等待扫码") + " (" + res.code + ")");
              }
            } else {
              setQrStatus((res && res.error) || "轮询失败");
            }
          }).catch(function () {});
        }, 2000);
      ...
      function startQr() {
        setQrLoading(true);
        setQrStatus("");
        api("GET", "/api/netease/qr-key").then(function (res) {
          setQrLoading(false);
          if (res && res.ok) {
            setQrUrl(res.url || "");
            setQrStatus("等待扫码");
          } else {
            setQrStatus((res && res.error) || "获取二维码失败");
          }
        }).catch(function () { setQrLoading(false); setQrStatus("获取二维码失败"); });
      }
      ...
      function playSong(song) {
        ...
        api("GET", "/api/netease/song-url?id=" + song.id).then(function (res) {
          setLoadId(null);
          if (res && res.ok) { setCur({ song: song, url: "/api/netease/proxy?id=" + song.id }); }
      ...
      var panel = isOpen ? e("div", { className: "nm-panel", style: { position: "fixed", left: pos.x, top: pos.y, width: 380, maxHeight: "min(82vh, 720px)", zIndex: 9999, pointerEvents: "auto" } },
        e("div", { className: "nm-panel-head", style: { cursor: "move", userSelect: "none", WebkitUserSelect: "none" }, onMouseDown: function (ev) { startDrag(ev, pos, setPos, 120, 44); } },
          e("div", { className: "nm-title" }, "网易云音乐" + (nick ? " · " + nick : "")),
          e("div", { style: { display: "flex", gap: 6, alignItems: "center" } },
            e("button", { className: "nm-close", onMouseDown: stop, onClick: function () { setMini(true); }, title: "最小化" }, "—"),
            e("button", { className: "nm-close", onMouseDown: stop, onClick: function () { setOpen(false); }, title: "关闭" }, "×")
          )
        ),
        e("div", { className: "nm-body" },
          showLoginVal ? e("div", { className: "nm-login" },
            e("div", { className: "nm-hint" }, "登录后可播放「我喜欢的音乐」"),
            e("div", { className: "nm-row" },
              e("button", { className: "nm-btn", onClick: startQr }, "扫码登录"),
              e("button", { className: "nm-btn ghost", onClick: function () { setShowLogin(false); } }, "稍后")
            ),
            qrLoadingVal ? e("div", { className: "nm-hint" }, "获取二维码中...") : null,
            qrUrlVal ? e("div", { className: "nm-qr" },
              e("div", { dangerouslySetInnerHTML: { __html: qrSvg(qrUrlVal) } }),
              e("div", { className: "nm-hint" }, "用网易云音乐 App「扫一扫」登录 · 协议 eapi/type3"),
              e("div", { className: "nm-status" }, qrStatusVal)
            ) : null,
            e("div", { className: "nm-sep" }, "— 或使用 Cookie 登录 —"),
            ...
            e("div", { className: "nm-row" },
              e("button", { className: "nm-btn ghost", onClick: doLogin }, "Cookie 登录")
            ),
      ...
      var miniBar = e("div", {
        className: "nm-mini",
        style: { position: "fixed", left: miniPos.x, top: miniPos.y, zIndex: 9999, pointerEvents: "auto", display: "flex", alignItems: "center", gap: 8, width: 276, padding: "8px 10px", background: "rgba(24,26,32,.97)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 10, boxShadow: "0 8px 24px rgba(0,0,0,.45)", color: "#e8e8ea", fontSize: 12, cursor: "move", userSelect: "none", WebkitUserSelect: "none", boxSizing: "border-box" },
        onMouseDown: function (ev) { startDrag(ev, miniPos, setMiniPos, 80, 30); }
      },
        e("span", { style: { color: "#e0483e", fontWeight: 700, flex: "none" } }, "♪"),
        e("div", { style: { flex: 1, minWidth: 0, lineHeight: 1.3 } },
          cur ? [
            e("div", { key: "n", style: { whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, cur.song.name),
            e("div", { key: "a", style: { fontSize: 10, color: "#8a8a92", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, cur.song.artists)
          ] : e("div", { style: { color: "#8a8a92" } }, "网易云音乐")
        ),
        e("button", { className: "nm-pbtn", style: { flex: "none" }, onMouseDown: stop, onClick: togglePlay, title: isPlaying ? "暂停" : "播放" }, isPlaying ? "||" : ">"),
        e("button", { className: "nm-pbtn", style: { flex: "none" }, onMouseDown: stop, onClick: function () { setMini(false); }, title: "展开" }, "▢"),
        e("button", { className: "nm-close", style: { flex: "none", fontSize: 16 }, onMouseDown: stop, onClick: function () { setOpen(false); setMini(false); }, title: "关闭" }, "×")
      );

      return e("div", { style: { pointerEvents: "none" } }, audioEl, isOpen ? (isMini ? miniBar : panel) : null);
    }
