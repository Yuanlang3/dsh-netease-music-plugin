...
// Turn a raw cookie payload (possibly Set-Cookie headers with attributes) into
// a clean "k=v; k2=v2" string, dropping cookie attributes (Path, Expires, ...)
// and attribute-only tokens (HttpOnly, Secure).
function cleanCookieString(str) {
  ...
  for (const part of parts) {
    ...
    if (["path", "domain", "expires", "max-age", "samesite", "priority"].includes(key)) continue;
    kept.push(p);
  }
  return sanitizeCookie(kept.join("; "));
}
...
// ---------------------------------------------------------------------------
// Audio proxy. The browser cannot play the Netease CDN URLs directly:
//  - mixed content (http:// on the http://127.0.0.1 GUI, a secure context)
//  - cross-origin 403 on restricted authSecret URLs (they need the account
//    cookie / music.163.com referer, which the browser never sends)
// The host therefore streams the audio itself: same-origin <audio src>,
// upstream request carries the account cookie + referer, Range is forwarded
// for seeking, and URLs are cached because they expire (~20 min).
// ---------------------------------------------------------------------------
const SONG_URL_TTL = 10 * 60 * 1000;
const songUrlCache = new Map();

function getCachedSongUrl(id) {
  const e = songUrlCache.get(id);
  if (e && Date.now() - e.ts < SONG_URL_TTL) return e.url;
  songUrlCache.delete(id);
  return null;
}
...
async function fetchSongUrl(id) {
  ...
  if (d && d.url) {
    ...
    songUrlCache.set(id, { url, ts: Date.now() });
    return url;
  }
  return null;
}

// Stream one URL to res. Resolves with the upstream status; on 403 nothing is
// written to res so the caller can retry with a fresh URL.
function streamUrl(res, url, range) {
  return new Promise((resolve) => {
    ...
    req.on("error", () => {
      try { writeJson(res, 200, { ok: false, error: "代理音频失败" }); } catch {}
      resolve(0);
    });
    req.end();
  });
...
async function handleProxy(req, res) {
  ...
  let url = getCachedSongUrl(id) || (await fetchSongUrl(id));
  if (!url) return writeJson(res, 200, { ok: false, error: "无法获取播放地址" });
  let status = await streamUrl(res, url, range);
  if (status === 403) {
    // The URL was probably issued for a stale session or just expired;
    // invalidate and retry once with a freshly fetched URL.
    songUrlCache.delete(id);
    url = await fetchSongUrl(id);
    if (url) status = await streamUrl(res, url, range);
  }
  if (status === 403) {
    try { writeJson(res, 403, { ok: false, error: "播放受限（403）" }); } catch {}
  }
}
...
async function handleQrCheck(req, res) {
  ...
  if (data.code === 803 || m !== null) {
    if (m) {
      // Store the full cookie string from the login response (MUSIC_U plus any
      // companion cookies). A bare MUSIC_U makes player/url return restricted
      // authSecret URLs that the browser cannot play (403).
      cookie = cleanCookieString(bodyCookie || headerCookie) || "MUSIC_U=" + m[1];
      profile = { userId: null, nickname: "", avatarUrl: "" };
    }
...
export function apply(ctx) {
  const routes = [
    ...
    { kind: "exact", path: "/api/netease/proxy", handler: handleProxy },
