...
async function handleQrKey(req, res) {
  const r = await weapiPost("https://music.163.com/weapi/login/qrcode/unikey", { type: 1 });
  let data = {};
  ...
  return writeJson(res, 200, { ok: false, error: data.message || "获取二维码失败" });
}
...
async function handleQrCheck(req, res) {
  ...
  const r = await weapiPost("https://music.163.com/weapi/login/qrcode/client/login", { key, type: 1 });
  let data = {};
  try { data = JSON.parse(r.bodyText); } catch {}
  if (data.code === 803) {
    const all = (r.setCookie || []).join("; ");
    const m = /MUSIC_U=([^;]+)/.exec(all);
    if (m) {
      cookie = "MUSIC_U=" + m[1];
      profile = { userId: null, nickname: "", avatarUrl: "" };
    }
