...
// Plain (unencrypted) form POST, used by the QR login endpoints. The current
// server accepts /api/login/qrcode/* as plain form posts; the type field must
// be 3 (new client). type:1 is rejected after the phone authorizes with
// code 8821 "请切换其他登录方式或升级新版本再试" (login freeze, fixed upstream
// in NeteaseCloudMusicApiEnhanced/api-enhanced).
function plainPost(url, data) {
  return formPost(url, new URLSearchParams(data).toString());
}
...
// QR-code login: issue a unikey, then poll the client/login endpoint. On
// success (code 803) the response's Set-Cookie carries MUSIC_U, which becomes
// the in-memory cookie used by every other route.
async function handleQrKey(req, res) {
  const r = await plainPost("https://music.163.com/api/login/qrcode/unikey", { type: 3 });
  let data = {};
...
async function handleQrCheck(req, res) {
  ...
  const r = await plainPost("https://music.163.com/api/login/qrcode/client/login", { key, type: 3 });
  let data = {};
  try { data = JSON.parse(r.bodyText); } catch {}
